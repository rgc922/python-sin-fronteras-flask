...
### definida en video 187 minuto 10 aprox
@bp.route('/')
@login_required
def index():
    db, c = get_db()
...
def update(id):
    ...
    if request.method == 'POST':
        ...
        error = None
        print(description, completed, error)
        ...
        if error is not None:
            ...
            ### el espacio antes de where es muy importante
            ## se hace cambio de línea con las comillas
            ## y lo estaba tomando pegado
            c.execute(
